...
--  Réinstallation propre : peut être relancé sans risque (projet neuf).
-- ============================================================

-- ---------- Réinitialisation (projet neuf, aucune donnée réelle) ----------
drop table if exists public.subscriptions cascade;
drop table if exists public.adhesion_keys cascade;
drop table if exists public.library_docs  cascade;
...
-- ---------- Clés d'adhésion ----------
create table public.adhesion_keys (
...
-- ---------- Bibliothèque (PDF dans le Storage bucket "bibliotheque") ----------
...
-- ---------- Abonnements (user_id -> profiles pour l'affichage admin) ----------
...
-- ---------- RPC : utiliser une clé d'adhésion (après inscription) ----------
create or replace function public.redeem_adhesion_key(p_code text, p_universite text, p_faculte text)
returns json language plpgsql security definer set search_path = public as $$
declare k public.adhesion_keys;
begin
  if auth.uid() is null then
    return json_build_object('ok', false, 'reason', 'Non authentifié.');
  end if;
  select * into k from public.adhesion_keys where code = upper(trim(p_code)) for update;
  if not found then
  ...
  update public.adhesion_keys set statut='utilisée', used_by=auth.uid(), used_at=now() where id=k.id;
  update public.profiles set role='membre', universite=p_universite, faculte=p_faculte, adhesion_key=k.code
    where id = auth.uid();
  return json_build_object('ok', true);
end;
$$;

-- ---------- RPC : vérifier une clé sans la consommer ----------
create or replace function public.check_adhesion_key(p_code text)
returns json language plpgsql security definer set search_path = public as $$
declare k public.adhesion_keys;
begin
  select * into k from public.adhesion_keys where code = upper(trim(p_code));
  if not found then
    return json_build_object('ok', false, 'reason', $r$Cette clé d'adhésion n'existe pas.$r$);
  end if;
  if k.statut = 'annulée' then
...
create or replace function public.activate_subscription(p_ref text, p_methode text, p_montant int)
returns json language plpgsql security definer set search_path = public as $$
declare v_fin date := current_date + interval '1 year';
begin
  if auth.uid() is null then return json_build_object('ok', false, 'reason', 'Non authentifié.'); end if;
...
create policy news_read  on public.news for select using (published or public.is_admin());
create policy news_write on public.news for all using (public.is_admin()) with check (public.is_admin());

create policy lib_read  on public.library_docs for select using (true);
create policy lib_write on public.library_docs for all using (public.is_admin()) with check (public.is_admin());

create policy subs_select on public.subscriptions for select using (user_id = auth.uid() or public.is_admin());
create policy subs_write  on public.subscriptions for all using (public.is_admin()) with check (public.is_admin());
...
À l'ordre du jour : présentation du bilan moral et financier de l'année écoulée, renouvellement partiel du bureau exécutif, et échanges libres avec les membres.
...
Après paiement de la carte, une clé d'adhésion personnelle vous sera remise : elle vous permettra de créer votre compte sur le portail AEEIG.$t$),
...
Les équipes doivent être constituées par université et inscrites auprès du secrétariat avant le 30 juin.$t$),
 ('resultats-bourses-excellence', $t$Bourses d'excellence : félicitations à nos lauréats$t$, '2026-05-28', 'Académique', 'cover-3',
  $t$Cinq étudiants membres de l'AEEIG figurent parmi les lauréats des bourses d'excellence.$t$,
  $t$L'AEEIG adresse ses chaleureuses félicitations aux cinq étudiants membres distingués cette année par les bourses d'excellence.$t$),
 ('accueil-nouveaux-bacheliers', $t$Accueil des nouveaux bacheliers : le guide d'installation à Conakry$t$, '2026-05-15', 'Entraide', 'cover-6',
  ...
  $t$Pour faciliter l'installation des nouveaux bacheliers ivoiriens, l'AEEIG publie un guide pratique et un dispositif de parrainage.$t$),
 ('partenariat-bibliotheque-numerique', $t$La bibliothèque numérique AEEIG s'enrichit de nouveaux mémoires$t$, '2026-05-02', 'Bibliothèque', 'cover-5',
  ...
  $t$La bibliothèque numérique continue de s'enrichir : une vingtaine de mémoires et thèses récemment soutenus viennent d'y être ajoutés, avec l'accord de leurs auteurs.$t$);
...
insert into public.library_docs (titre, auteur, type, filiere, annee, resume) values
 ...
 ($t$Le droit OHADA et la protection des investisseurs étrangers$t$, 'Kouassi Aya', 'Mémoire', 'Droit', 2025, $t$Analyse comparée des mécanismes de protection dans l'espace OHADA.$t$),
 ($t$Impact du mobile money sur l'inclusion financière en Guinée$t$, 'Traoré Ibrahim', 'Thèse', 'Économie', 2024, $t$Thèse de doctorat : enquête auprès de 1 200 ménages dans quatre régions.$t$),
 ($t$Cours complet d'anatomie — PCEM2$t$, 'Pr. Diallo M.', 'Cours', 'Médecine', 2026, $t$Support de cours d'anatomie de deuxième année, illustré.$t$),
 ($t$Guide d'installation des étudiants ivoiriens à Conakry$t$, 'AEEIG', 'Guide', 'Vie pratique', 2026, $t$Démarches administratives, logement, transport, santé.$t$),
 ($t$Béton armé : dimensionnement selon les Eurocodes$t$, 'Yao Kouadio', 'Cours', 'Génie civil', 2025, $t$Synthèse de cours et exercices corrigés en génie civil.$t$),
 ...
 ($t$Introduction au droit constitutionnel guinéen$t$, 'Dr. Camara S.', 'Livre', 'Droit', 2023, $t$Manuel de référence pour la première année de droit.$t$),
 ($t$Méthodologie de la recherche : rédiger son mémoire$t$, 'AEEIG', 'Guide', 'Méthodologie', 2025, $t$De la problématique à la soutenance : le guide méthodologique.$t$),
 ($t$Microéconomie L2 : théorie du consommateur et du producteur$t$, 'Dr. Koné B.', 'Cours', 'Économie', 2026, $t$Polycopié complet avec exercices corrigés (licence 2).$t$),
 ($t$Santé communautaire et vaccination en zone rurale$t$, 'Bamba Mariam', 'Mémoire', 'Médecine', 2024, $t$Enquête de terrain dans la région de Kindia.$t$),
 ($t$L'arbitrage commercial international dans l'espace OHADA$t$, 'Koné Salif', 'Mémoire', 'Droit', 2023, $t$Étude de la CCJA et de sa jurisprudence récente.$t$);
